...
#--------------------------------------------------------------------------------------------------
def test_plot(matrix_map, threshold, exploration): #TODO: remove afterwards...This is just for quick validations
    ...
    plt.scatter(ex_x, ex_y, s = 0.35, color = 'purple')
...
def generate_node(parent_ctc, parent_state, unvisited_list, matrix_map, action, step_size,\
    ...
    dt = 0.5    # Fine tune dt as per need
    t = 0
...
def run_A_star(initial_x, initial_y, final_x, final_y, initial_orientation, threshold, matrix_map,\
    ...
    node_parent = (-1000, -1000, 0) # Just for the start node
    ...
    while ((unvisited_list != []) and (goal_reached != "yes")):
        ...
        if goal_reached != "yes":
            ...
            test_plot(matrix_map, threshold, visited_list)
...
def runner():
    ...
    final_x, final_y = 20,100#map(int,input("Enter the final x and y position in cm separated by a space: ").split())
